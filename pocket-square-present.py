...
def index(user_id):
    ...
    posts_request = requests.get('http://188.166.174.189:28104/sort/' + user_id)
    posts = posts_request.json()

    for post in posts:
        post['similar_url'] = 'http://0.0.0.0:4001/index/%s/similar/%s' % (user_id, post['id'])
...
def similar(user_id, text_id):
    # posts_request = requests.get('http://pocket_square_sort_shuffle:5000/sort/' + user_id)
    posts_request = requests.get('http://0.0.0.0:5001/%s/%s/similar_by_text' % (user_id, text_id))
    posts = posts_request.json()
